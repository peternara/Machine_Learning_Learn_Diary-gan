...
You can replace the buckets argument with any folder containing subfolders of
...
def create_image_lists(buckets, testing_percentage, validation_percentage):
    ...
    Args:
      buckets: String path to a folder containing subfolders of images.
    ...
    """
    if not tf.gfile.Exists(buckets):
        print("Image directory '" + buckets + "' not found.")
        return None
    result = {}
    sub_dirs = [x[0] for x in tf.gfile.Walk(buckets)]
    ...
    for sub_dir in sub_dirs:
        ...
        for extension in extensions:
            file_glob = os.path.join(buckets, dir_name, '*.' + extension)
...
def get_image_path(image_lists, label_name, index, buckets, category):
    ...
    Args:
      ...
      buckets: Root folder string of the subfolders containing the training
      images.
    ...
    full_path = os.path.join(buckets, sub_dir, base_name)
    return full_path
...
def create_inception_graph():
    ...
    with tf.Graph().as_default() as graph:
        model_filename = os.path.join(
            FLAGS.checkpointDir, 'classify_image_graph_def.pb')
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
                           buckets, category, sess, jpeg_data_tensor,
                           bottleneck_tensor):
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, buckets,
...
def cache_bottlenecks(sess, image_lists, buckets, bottleneck_dir,
        ...
        for category in ['training', 'testing', 'validation']:
            ...
            for index, unused_base_name in enumerate(category_list):
                get_or_create_bottleneck(sess, image_lists, label_name, index,
                                         buckets, category, bottleneck_dir,
                                         jpeg_data_tensor, bottleneck_tensor)
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
                                  bottleneck_dir, buckets, jpeg_data_tensor,
                                  bottleneck_tensor):
...
def get_random_distorted_bottlenecks(
        sess, image_lists, how_many, category, buckets, input_jpeg_tensor,
    ...
    for unused_i in range(how_many):
        ...
        label_name = list(image_lists.keys())[label_index]
        image_index = random.randrange(MAX_NUM_IMAGES_PER_CLASS + 1)
        image_path = get_image_path(image_lists, label_name, image_index, buckets,
                                    category)
        if not tf.gfile.Exists(image_path):
            tf.logging.fatal('File does not exist %s', image_path)
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
    ...
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            ...
            l2_loss_h1 = tf.nn.l2_loss(initial_value_h1)
            l2_loss_h1 = tf.multiply(l2_loss_h1, 4e-3, 'l2_loss_h1')
            ...
            l2_loss_h2 = tf.nn.l2_loss(initial_value_h2)
            l2_loss_h2 = tf.multiply(l2_loss_h2, 4e-3, 'l2_loss_h2')
            ...
            l2_loss = tf.nn.l2_loss(initial_value)
            l2_loss = tf.multiply(l2_loss, 4e-3, 'l2_loss')
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        if do_distort_images:
            ...
            # cached them on disk.
            cache_bottlenecks(sess, image_lists, FLAGS.buckets,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--summaryDir',
        type=str,
        default='summaryDir/retrain_logs',
